...
using Random
using Distributed; addprocs(2) # Add one Julia worker per CPU core
using Dagger
end

# ╔═╡ 256e824a-4835-47ae-b92c-d67d8552b36a
md"""
# Composable data reduction algorithms using distributed task parallelism
"""
...
N, M = 1000, 200 #10000, 1000 # No. of samples(rows) and no. of features(cols).
...
# ╔═╡ 75384263-dbe6-417b-8a64-72d27ab2547d
Dm = distribute(m, Blocks(N÷2, M))

# ╔═╡ a08d9ca6-f9c5-45a9-893b-99640dccb433
Db = distribute(b, Blocks(N÷2))
...
# ╔═╡ 7ef2701b-aeaf-464e-9c60-64a0a16768ad
N′ = 100 # reduced no. of features(cols)

# ╔═╡ e906a8e7-ed69-42fb-8790-e7b1c59f206f
md"""
Apply CUR to the dataset.
"""

# ╔═╡ e8851b24-c06f-4b89-8a8a-6b9d7eb57ef9
rows, cols = cur(Dm)

# ╔═╡ b3876bd4-2486-4c2d-ae82-906a10f57f91
m_cur = Dm[rows, :]
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "1a9e4297e3276766964cf3980555a1a22d8bd7fc"
